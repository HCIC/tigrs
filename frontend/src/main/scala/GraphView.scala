...
case class GraphProps(charge: Double)

object GraphView extends D3ForceLayout[graph.Vertex, GraphProps] {
  type V = graph.Vertex
  type G = DirectedGraph[V]

  override def apply(graph: DirectedGraph[V], width: Double, height: Double, props: Option[GraphProps]) = component(Props(graph, width, height, props))
  ...
  override def linkDistance(p: Props, e: Edge[V]) = {
    import p.graph._
    10 * log((degree(e.in) max degree(e.out)) + 2)
  }
  override def linkStrength(p: Props, e: Edge[V]): Double = 5
